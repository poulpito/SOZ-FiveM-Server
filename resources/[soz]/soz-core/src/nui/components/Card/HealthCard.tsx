...
export const HealthCard: FunctionComponent<HealthCardProps> = ({ player }) => {
    return (
        ...
        >
            <div className={`flex ${isStandardScreen ? 'pt-[6vh] pl-[5vh]' : 'pt-[5vh] pl-[4vh]'} uppercase`}>
                <div className={`${isStandardScreen ? 'w-[14vh]' : 'w-[11vh]'}`}>{player.charinfo.lastname}</div>
                <div>{player.charinfo.firstname}</div>
            </div>
            <p
                className={`${
                    isStandardScreen ? 'mt-[1.3vh] ml-[20vh]' : 'mt-[1.1vh] ml-[15vh]'
                } uppercase italic font-bold`}
            >
                {healthLevelToLabel(player.metadata.health_book_health_level, 0, 100)}
            </p>
            <div className="flex justify-items-start text-sm -full mt-2 pl-[2vh]">
